  ...
  try {
    ...
    if (id) {
      ...
      const conflictSeason = allSeason.find((seasonItem) => {
        return (
          (new Date(start_date) >= seasonItem.start_date &&
            new Date(start_date) <= seasonItem.end_date) ||
          (new Date(end_date) >= seasonItem.start_date && new Date(end_date) <= seasonItem.end_date)
        );
      });
